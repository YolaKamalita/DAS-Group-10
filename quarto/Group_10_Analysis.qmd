...
Interestingly, the Animation's odds is 0.49 times to Action's odds, and this is not align with the fact that Animation's proportion (of having rating higher than 7) is bigger than Action. Then, we notice that Animation has a mean of length 19.92 min, which is lower than the overall mean, 82.29 min from @tbl-summary-stats. One hypothesis is that the model struggles in adequately balancing between length and genre variables during the model fitting process, with length gives a higher influence. However, Animation's odds is not statistically significant.

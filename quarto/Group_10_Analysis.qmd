...
Draw scatter plots of scores and variables for preliminary exploratory analysis
...
```{r}
# Calculate the correlation coefficient between all the variables in df
# Ensure that the data only contains numeric columns

numeric_df <- df[sapply(df, is.numeric)]
correlation_matrix <- cor(numeric_df, use = "complete.obs")# The 'use' parameter handles missing values
...
grid.arrange(p1, p2, p3, p4, ncol=2)#This part of the code has reported an error, but I can not solve it, I do not know what the problem is
p5
```
